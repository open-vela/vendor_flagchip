  ...
  if (port < FC7300_NPORTS)
    {
      ...
      if (mode == PIN_MODE_ANALOG)
      ...
      else
        {
          ...
          if ((cfgset & _PIN_IO_MASK) == _PIN_INPUT)
          ...
          else
            {
              ...
              if ((cfgset & _PIN_OUTPUT_DRIVE_MASK) == _PIN_OUTPUT_HIGHDRIVE)
                {
                  regval |= PORT_PCR_DSE0;
                  regval |= PORT_PCR_DSE1;
                }
            }
          ...
          regval = getreg32(base + FC7300_PORT_DFER_OFFSET);
          ...
          putreg32(regval, base + FC7300_PORT_DFER_OFFSET);
          ...
          regval = getreg32(base + FC7300_GPIO_PIDR_OFFSET);
          ...
          putreg32(regval, base + FC7300_GPIO_PIDR_OFFSET);
          ...
          if (mode == PIN_MODE_GPIO)
            {
              ...
              base   = FC7300_GPIO_BASE(port);
              regval = getreg32(base + FC7300_GPIO_PDDR_OFFSET);
              if ((cfgset & _PIN_IO_MASK) == _PIN_INPUT)
                {
                  /* Select GPIO input */

                  regval &= ~(1 << pin);
                  putreg32(regval, base + FC7300_GPIO_PDDR_OFFSET);
                }
              else /* if ((cfgset & _PIN_IO_MASK) == _PIN_OUTPUT) */
                {
                  ...
                  fc7300_gpiowrite(cfgset, (cfgset & GPIO_OUTPUT_MASK) != 0);
                }
            }
        }
 ...
 *
 * Input Parameters:
 *   port  - Port number.  See FC7300_PORTn definitions in fc7300_port.h
 *   cs   - true: Digital Filters are clocked by the bus clock
 *           false: Digital Filters are clocked by the AON32K clock
 *   width - Filter Length
 *
 ****************************************************************************/

int fc7300_pinfilter(unsigned int port, bool cs, unsigned int width)
{
  uintptr_t base;
  uint32_t  regval;
  ...
  if (port < FC7300_NPORTS)
    {
      ...
      regval = (cs ? PORT_DFCR_CS : 0);
      putreg32(regval, base + FC7300_PORT_DFCR_OFFSET);

      /* Select the filter width */

      DEBUGASSERT(width < 32);
      putreg32(width, base + FC7300_PORT_DFWR_OFFSET);
      return OK;
    }
