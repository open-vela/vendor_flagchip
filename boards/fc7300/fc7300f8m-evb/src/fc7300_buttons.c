...
/* Pin configuration for each FC4150F2M-EVB button. This array is indexed
  ...
  for (i = 0; i < NUM_BUTTONS; i++)
    {
      ...
      bool released = fc7300_gpioread(g_buttons[i]);
...
int board_button_irq(int id, xcpt_t irqhandler, void *arg)
{
  ...
  if (id >= MIN_IRQBUTTON && id < MAX_IRQBUTTON)
    {
      ret = fc7300_pinirqattach(g_buttons[id], irqhandler, NULL);
      if (ret >= 0)
        {
          /* Then make sure that interrupts are enabled on the pin */

          fc7300_pinirqenable(g_buttons[id]);
        }
    }
